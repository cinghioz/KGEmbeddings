...
def main(args):
    ...
    logging.info('Total triplets: %d' % len(kg.triplets))
    logging.info('#train: %d' % len(kg.train_set))
    logging.info('#valid: %d' % len(kg.valid_set))
    logging.info('#test: %d' % len(kg.test_set))

    kge_model = KGEModel(
        model_name=args.model,
        nentity=kg.number_of_entities,
        nrelation=kg.number_of_relations,
    ...
    if args.do_test:
        ...
        metrics = kge_model.test_step(kge_model, kg.triplets[kg.test_set], kg.triplets, args)
        log_metrics('Test', step, metrics)
    
    if args.evaluate_train:
        logging.info('Evaluating on Training Dataset...')
        metrics = kge_model.test_step(kge_model, kg.triplets[kg.train_set], kg.triplets, args)
